...
#'
#' @param params A numeric vector of parameters c(a1, b1, a2, b2)
#' @param musigma A numeric vector c(mu, sigma) for the latent trait distribution
#' @param sd Optional pooled standard deviation for standardization
#' @param cov_matrix Optional covariance matrix for standard error computation
#'
...
computeAreas <- function(params, musigma, sd = NULL, cov_matrix = NULL) {
   ...
   squared_f <- function(x) {
      diff <- (a1 - a2) + (b1 - b2) * x
      phi_x <- stats::dnorm(x, mean = mu, sd = sigma)  # Latent trait density
      (diff^2) * phi_x
   }
   ...
   if (!is.null(cov_matrix)) {
      ...
      unsigned_gradient <- numDeriv::grad(function(p) {
         ...
         a2 <- p[3]
         b2 <- p[4]
         stats::integrate(squared_f, lower = -Inf, upper = Inf)$value
      }, params)
      ...
      if (!is.null(xc)) {
         signed_gradient <- numDeriv::grad(function(p) {
            ...
            if (b1 != b2) {
               xc <- (a2 - a1) / (b1 - b2)
               above <- stats::integrate(squared_f, lower = xc, upper = Inf)$value
               below <- stats::integrate(squared_f, lower = -Inf, upper = xc)$value
